#!/usr/bin/env python3
"""Fail-fast A1 operator, parameter and two-output contract audit."""
...
def main() -> None:
    ...
    for node in model.graph.node:
        if node.op_type == "Conv":
            ...
            if weight is not None and len(weight.dims) == 4:
                cin_per_group, kh, kw = int(weight.dims[1]), int(weight.dims[2]), int(weight.dims[3])
                cin = cin_per_group * group
                if cin_per_group * kh * kw > 2048:
                    errors.append(f"{node.name or node.output[0]} Kw*Kh*Cin/group exceeds 2048")
                if group not in (1, cin):
                    errors.append(f"{node.name or node.output[0]} group={group}, Cin={cin}")
    ...
    report = {
        ...
        "note": "Final authority remains the official A1 conversion compiler.",
    }
