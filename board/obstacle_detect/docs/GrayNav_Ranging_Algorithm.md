...
对地面接触点本身，系统使用 `y_contact = y2 - clamp(0.012*h_box, 1, 8)`，补偿检测框在目标下方的回归 padding。该比例可通过 `A1_GROUND_CONTACT_OFFSET_RATIO` 标定。

### 4.5 远场投影误差传播

地面距离为 `z=h/tan(theta)`。对底边像素 `v` 求导后，将检测框底定位标准差 `sigma_v` 传播为：

```text
sigma_z,projection = |dz/dtheta| * |dtheta/dv| * sigma_v
```

当目标较远时，射线俯角趋近地平线，`|dz/dtheta|` 快速增大。因此系统不会再给远场目标虚假的高置信米级数值，而是增大 `sigma_z`，规划器继续使用 `z_safe=z-sigma_z`。
