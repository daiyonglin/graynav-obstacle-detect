...
每条轨迹维护长度等于模型类别数的 `class_evidence`。当前 Indoor8 模型的旧证据每帧乘 0.85，新类别加当前置信度；普通切换要求新类别连续出现 3 次且证据达到旧类别的 1.2 倍，高置信新类别可在连续 2 次后纠正。旧 ROD25 的 person-part bridge 在 8 类模型下强制关闭，防止椅子、桌子被历史 PERSON 轨迹自锁。

### 8.4 轨迹创建和删除

- 一般目标分数至少 0.22；person 可放宽到 0.08；
- Indoor8 中 person 连续命中 2 次后发布，椅子、桌子、包等连续命中 3 次后发布；
- coarse 或不合理宽框不能创建轨迹；
- 低置信轨迹至少命中 2 次才发布；
...
当前统一 Indoor8 模型只对尺寸相对稳定、且观测形态可靠的类别使用物理先验。完整目标使用：

```text
z_size = fy * H_real / h_pixel
```

人体只有在框未触碰上下边界、宽高比不大于 0.62、高度占画面至少 18%、宽度不超过 55% 时，才允许把框底当脚点并使用 1.70m 全身高度。面部、上半身、腿部或被裁断的人体框一律禁用地面交点和全身高度，改用可见宽度先验：常规肩/躯干约 0.40m，近方形头肩框约 0.32m，窄腿部框约 0.28m，并赋予 38%~45% 的较大相对方差。

`A1_RANGE_GEOMETRY_SCALE=1.60` 现在只校正依赖脚点的地面交点证据；尺寸证据单独使用 `A1_RANGE_SIZE_SCALE=1.00`。这项拆分直接针对“上半身框误套 1.70m，再乘 1.60，导致实距 1.2m 却显示约 3m”的故障。尺寸先验不是精确常数，代码始终同时产生 `mean` 和 `sigma`。

地面投影不再把框底 `y2` 直接当接地点，而是减去受限的框高比例偏移，补偿 YOLO 回归框在目标下方的 padding。距离方差由 `z=h/tan(theta)` 对底边像素的雅可比传播得到，远场小俯角下方差会自动增大。

### 9.4 近场上界

框底靠近图像底部且面积较大时，`NearFieldUpperBound()` 给出规划用的距离上界。对局部人体，宽度占画面 42% 或高度占 62% 时上界为 0.95m，宽度 30% 或高度 48% 时为 1.20m，宽度 20% 或高度 34% 时为 1.80m。它不改写串口展示的期望距离，只防止滤波历史或宽度先验方差把一个画面占比很大的近场人体释放成 FAR/CLEAR。贴住左右边界、宽度超过 90% 或 coarse 框仍不能触发旧式强近场规则，避免横框制造虚假紧急距离。
...
冲突时保留地面证据并扩大 1.35 倍方差。统一模型的 16 级学习深度可在尺度锚点可靠时参与局部人体展示值，权重为几何宽度证据 0.45、学习深度 0.55；局部人体本身禁止成为尺度锚点。规划值始终取各证据的更保守下界，学习深度不能把风险距离放大。规划使用：
