...
`CandidateThreshold(raw_class_id)` 控制召回阶段阈值；`RiskWeight(semantic_class_id)` 只参与后处理排序和规划，不修改模型分数。更换模型类别时，应集中修改本文件，并同步 CMake 的 `A1_YOLO_NUM_CLASSES`。
...
为抑制人体框在“全身框/上半身框/头像框”之间切换造成的距离突跳，tracker 对突然变远的单帧测量先保持预测距离，只有连续两次远距离测量一致才接纳；突然变近的测量立即接纳，以保持避障的保守性和响应速度。
...
风险阈值为：TTC 小于 1.5s 或安全距离小于 0.8m 为 urgent；小于 1.05m 为 near；小于 2.0m 为 warning；否则 far。
